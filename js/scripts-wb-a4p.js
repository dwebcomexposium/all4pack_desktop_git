...
var $wrappedText           = $('.at-main-title, .article-wrapper h4, h1');
...
if ($('.cxp-pagination').length) {
	var observer = new MutationObserver(function() {
		if (!$('.cxp-pagination .container').length) {
			$('.cxp-pagination .la-item-img').wrap('<div class="container"></div>');
		}
	});

	observer.observe($('.cxp-pagination').get(0), {
		childList: true,
		subtree: true
	});
}
